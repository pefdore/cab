...
function setDefaultDate() {
    ...
    const fluxYearSelect = document.getElementById('flux-year-select');
    if (fluxYearSelect) {
        fluxYearSelect.value = new Date().getFullYear().toString();
    }
}
...
function deletePDF(id) {
    ...
    // Use nice modal (works on both PC and mobile now)
...
window.saveOpenRouterKey = async function() {
    ...
    if (currentUser && supabaseClient) {
        ...
        console.log('[API Key] Cannot save to Supabase - currentUser or supabaseClient undefined');
    }
...
function renderFluxView() {
    const yearSelect = document.getElementById('flux-year-select');
    const typeSelect = document.getElementById('flux-type-select');
    const categorySelect = document.getElementById('flux-category-select');
    ...
    const depenses = cabinetDepenses.filter(d => {
        const dDate = new Date(d.date);
        return dDate.getFullYear().toString() === year;
    });
    
    const recettes = cabinetRecettes.filter(r => {
        const rDate = new Date(r.date);
        return rDate.getFullYear().toString() === year;
    });
    ...
    if (typeFilter !== 'all') {
        allFlux = allFlux.filter(f => f.type === typeFilter);
    }
    
    if (categoryFilter !== 'all') {
        allFlux = allFlux.filter(f => f.category === categoryFilter);
    }
    
    allFlux.sort((a, b) => new Date(b.date) - new Date(a.date));
    
    const totalRecettes = recettes.reduce((sum, r) => sum + (parseFloat(r.amount) || 0), 0);
    const totalDepenses = depenses.reduce((sum, d) => sum + (parseFloat(d.amount) || 0), 0);
    const balance = totalRecettes - totalDepenses;
    
    const totalIncomeEl = document.getElementById('flux-total-income');
    const totalExpenseEl = document.getElementById('flux-total-expense');
    const balanceEl = document.getElementById('flux-balance');
    
    if (totalIncomeEl) totalIncomeEl.textContent = totalRecettes.toFixed(0) + '€';
    if (totalExpenseEl) totalExpenseEl.textContent = totalDepenses.toFixed(0) + '€';
    if (balanceEl) {
    ...
    tbody.innerHTML = allFlux.map(f => {
        const fDate = new Date(f.date);
        const isRecette = f.type === 'recette';
        const categoryLabel = isRecette 
            ? (RECETTE_CATEGORIES[f.category]?.label || f.category)
            : (CABINET_CATEGORIES[f.category]?.label || f.category);
        
        return `
            <tr>
                <td>${fDate.toLocaleDateString('fr-FR', {day:'numeric', month:'numeric', year:'numeric'})}</td>
                <td><span class="flux-type-badge ${f.type}">${isRecette ? 'Entrée' : 'Sortie'}</span></td>
                <td class="flux-category">${categoryLabel}</td>
                <td>${f.description || '-'}</td>
                <td class="text-right">
                    <span class="flux-amount ${f.type}">${isRecette ? '+' : '-'}${parseFloat(f.amount).toFixed(2)}€</span>
                </td>
                <td>
                    <button class="btn-icon btn-delete" onclick="deleteFluxItem('${f.type}', ${f.id})" title="Supprimer">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                        </svg>
                    </button>
    ...
    renderFluxMonthlyView(year, depenses, recettes);
}

function renderFluxMonthlyView(year, depenses, recettes) {
    const container = document.getElementById('flux-monthly-grid');
    if (!container) return;
    ...
    for (let m = 0; m < 12; m++) {
        ...
        const monthRecettes = recettes.filter(r => {
            const rDate = new Date(r.date);
            return rDate.getMonth() === m;
        });
        
        const totalDepenses = monthDepenses.reduce((sum, d) => sum + (parseFloat(d.amount) || 0), 0);
        const totalRecettes = monthRecettes.reduce((sum, r) => sum + (parseFloat(r.amount) || 0), 0);
        const balance = totalRecettes - totalDepenses;
        
        html += `
            <div class="flux-month-card">
                <div class="flux-month-card-header">
                    <span class="flux-month-name">${months[m]}</span>
                    <span class="flux-month-balance ${balance >= 0 ? 'positive' : 'negative'}">${balance >= 0 ? '+' : ''}${balance.toFixed(0)}€</span>
                </div>
                <div class="flux-month-details">
                    <div class="flux-month-detail income">
                        <span>Entrées:</span>
                        <span>+${totalRecettes.toFixed(0)}€</span>
                    </div>
                    <div class="flux-month-detail expense">
                        <span>Sorties:</span>
                        <span>-${totalDepenses.toFixed(0)}€</span>
                    </div>
                </div>
            </div>
        `;
...
window.switchFluxView = function(view) {
    const btns = document.querySelectorAll('.flux-toggle-btn');
    btns.forEach(btn => {
        btn.classList.toggle('active', btn.dataset.fluxView === view);
    });
    
    const listView = document.getElementById('flux-list-view');
    const monthlyView = document.getElementById('flux-monthly-view');
...
window.deleteFluxItem = async function(type, id) {
    if (!confirm('Supprimer cette ' + (type === 'recette' ? 'recette' : 'dépense') + ' ?')) return;
    
    try {
        const table = type === 'recette' ? 'cabinet_recettes' : 'cabinet_depenses';
...
window.openFluxAddModal = function(type) {
    const modal = document.getElementById('flux-add-modal');
    if (!modal) {
        const newModal = document.createElement('div');
        newModal.id = 'flux-add-modal';
        ...
        newModal.innerHTML = `
            <div class="modal-card" style="max-width: 500px;">
                <div class="modal-header">
                    <h3>${isDepense ? 'Ajouter une dépense' : 'Ajouter une recette'}</h3>
                    <button class="modal-close" onclick="closeModal('flux-add-modal')">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
                        </svg>
                    </button>
                ...
                <div class="modal-body">
                    <form id="flux-add-form" class="flux-modal-form">
                        <div class="form-group">
                            <label>Description</label>
                            <input type="text" id="flux-add-description" placeholder="${isDepense ? 'Ex: Loyer, EDF...' : 'Ex: Honoraires, Remboursement...'}" required>
                        </div>
                        <div class="form-group">
                            <label>Catégorie</label>
                            <select id="flux-add-category" required>
                                ${isDepense ? `
                                    <option value="masse_salariale">Masse salariale</option>
                                    ...
                                    <option value="reception">Réception/Représentation</option>
                                ` : `
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label>Montant</label>
                                <input type="number" id="flux-add-amount" placeholder="0.00" step="0.01" required>
                            </div>
                            <div class="form-group">
                                <label>Date</label>
                                <input type="date" id="flux-add-date" required>
                            </div>
                        </div>
        ...
        const today = new Date().toISOString().split('T')[0];
        document.getElementById('flux-add-date').value = today;
        
        document.getElementById('flux-add-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            await saveFluxItem(type);
        });
        
        newModal.addEventListener('click', function(e) {
            if (e.target === this) closeModal('flux-add-modal');
        });
        
        return;
    }
    
    const form = document.getElementById('flux-add-form');
    if (form) {
        form.reset();
        const today = new Date().toISOString().split('T')[0];
        document.getElementById('flux-add-date').value = today;
        
        const categorySelect = document.getElementById('flux-add-category');
        if (categorySelect) {
            categorySelect.innerHTML = type === 'depense' 
                ? `<option value="masse_salariale">Masse salariale</option>
                   <option value="urssaf">URSSAF</option>
                   <option value="logiciel">Logiciel</option>
                   <option value="services">Services</option>
        ...
        if (header) {
            header.textContent = type === 'depense' ? 'Ajouter une dépense' : 'Ajouter une recette';
        }
    }
...
async function saveFluxItem(type) {
    const description = document.getElementById('flux-add-description').value;
    const category = document.getElementById('flux-add-category').value;
    const amount = parseFloat(document.getElementById('flux-add-amount').value);
    const date = document.getElementById('flux-add-date').value;
    ...
    try {
        const table = type === 'depense' ? 'cabinet_depenses' : 'cabinet_recettes';
        ...
        closeModal('flux-add-modal');
        
        await loadCabinetData();
        
        if (document.getElementById('view-flux')?.classList.contains('active')) {
            renderFluxView();
        }
