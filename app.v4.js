...
function applyCotationVisibility() {
    ...
    const cotationPrefItem = document.getElementById('cotation-preference-item');
    const fabPassageItem = document.getElementById('fab-menu-item-passage');
    ...
    if (cotationPrefItem) {
        cotationPrefItem.style.display = cotationEnabled ? '' : 'none';
    }
    
    if (fabPassageItem) {
        fabPassageItem.style.display = cotationEnabled ? '' : 'none';
    }
    ...
    if (typeof switchDashboardMode === 'function') {
        switchDashboardMode(cotationEnabled ? 'cotation' : 'cabinet');
    }
}
...
function initCotationVisibility() {
    const checkAndApply = () => {
        ...
        if (cotationPrefItem) {
            cotationPrefItem.style.display = isEnabled ? '' : 'none';
        }
        
        if (fabPassageItem) {
            fabPassageItem.style.display = isEnabled ? '' : 'none';
        }
        ...
        if (typeof switchDashboardMode === 'function') {
            switchDashboardMode(isEnabled ? 'cotation' : 'cabinet');
        }
    };
